...
| `interactionEventTS`  | number  | When this event occurred (seconds since epoch). Absent on some event types                       |
...
| `eventTS`             | number  | When the interaction was offered (seconds since epoch). Same on every event for that interaction |
...
- Calculate durations using `interactionEventTS`, not `eventTS` (see
